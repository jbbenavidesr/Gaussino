    ...
    namespace mpmc_sema {
      ...
      public:
        ...
        void signal( int count ) {
          while ( count-- > 0 ) { semaphore_signal( m_sema ); }
        }
      };
#elif defined( __unix__ )
      //---------------------------------------------------------
...
#  error Unsupported platform! (No semaphore wrapper available)
      ...
      public:
        ...
        void signal( ssize_t count = 1 ) {
          ...
          if ( toRelease > 0 ) { m_sema.signal( (int)toRelease ); }
        }
  ...
  public:
    ...
    explicit BlockingConcurrentQueue( size_t capacity = 6 * BLOCK_SIZE )
      ...
      if ( !sema ) { MOODYCAMEL_THROW( std::bad_alloc() ); }
    }
  ...
  public:
    ...
    inline bool try_dequeue( U& item ) {
      if ( sema->tryWait() ) {
        while ( !inner.try_dequeue( item ) ) { continue; }
        return true;
      }
      return false;
    }
    ...
    inline bool try_dequeue( consumer_token_t& token, U& item ) {
      if ( sema->tryWait() ) {
        while ( !inner.try_dequeue( token, item ) ) { continue; }
        return true;
      }
      return false;
    }
    ...
    inline size_t try_dequeue_bulk( It itemFirst, size_t max ) {
      ...
      while ( count != max ) { count += inner.template try_dequeue_bulk<It&>( itemFirst, max - count ); }
      return count;
    }
    ...
    inline size_t try_dequeue_bulk( consumer_token_t& token, It itemFirst, size_t max ) {
      size_t count = 0;
      max          = (size_t)sema->tryWaitMany( ( LightweightSemaphore::ssize_t )(ssize_t)max );
      while ( count != max ) { count += inner.template try_dequeue_bulk<It&>( token, itemFirst, max - count ); }
      return count;
    }
