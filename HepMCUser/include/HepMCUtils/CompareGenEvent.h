...
//
inline bool operator!=( const HepMC3::GenEvent& left, const HepMC3::GenEvent& right ) { return !( left == right ); }

inline bool operator!=( const HepMC3::GenVertex& left, const HepMC3::GenVertex& right ) { return !( left == right ); }
...
inline std::ostream& operator<<( std::ostream& strm, const HepMC3::GenParticle& particle ) {
  ...
  strm << "   Momentum [ " << particle.momentum().px() << ", " << particle.momentum().py() << ", "
       << particle.momentum().pz() << ", " << particle.momentum().e() << "]\n";
  return strm;
}

inline std::ostream& operator<<( std::ostream& strm, const HepMC3::GenVertex& vertex ) {
  strm << " Vertex at [ " << vertex.position().x() << ", " << vertex.position().y() << ", " << vertex.position().z()
       << ", " << vertex.position().t() << "]\n";
  return strm;
}
