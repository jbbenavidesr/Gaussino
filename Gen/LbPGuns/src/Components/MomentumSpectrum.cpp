...
// Constructor
//===========================================================================
MomentumSpectrum::MomentumSpectrum( const std::string& type, const std::string& name, const IInterface* parent )
...
StatusCode MomentumSpectrum::initialize() {
  ...
  if ( m_binningVars == "pxpypz" ) {
    if ( m_hist->GetDimension() != 3 ) {
      error() << "BinningVariables set to " << m_binningVars << " but histogram " << m_histoPath << " has dimension "
              << m_hist->GetDimension() << endmsg;
      return StatusCode::FAILURE;
    }
    m_hist2d = 0;
  ...
  info() << "Using file '" << m_inputFileName << "' with histogram '" << m_histoPath
         << "' for sampling the momentum spectrum" << endmsg;
...
//===========================================================================
void MomentumSpectrum::generateParticle( Gaudi::LorentzVector& momentum, Gaudi::LorentzVector& origin, int& pdgId,
...
bool MomentumSpectrum::newEvent( const LHCb::GenHeader* evt ) {
  ...
  if ( runNr == m_runnumber && evtNr == m_evtnumber ) { return false; }
  if ( runNr != m_runnumber ) { m_runnumber = evt->runNumber(); }
  if ( evtNr != m_evtnumber ) { m_evtnumber = evt->evtNumber(); }
  return true;
}
