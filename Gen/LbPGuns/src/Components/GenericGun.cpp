...
// Constructor
//==========================================================================
GenericGun::GenericGun( const std::string& type, const std::string& name, const IInterface* parent )
...
StatusCode GenericGun::initialize() {
  ...
  if ( SPGGenMode::FixedMode != m_PtGenMode ) {
    if ( ( m_minPt > m_requestedPt ) || ( m_maxPt < m_requestedPt ) || ( m_maxPt < m_minPt ) ) {
  ...
  if ( SPGGenMode::FixedMode != m_EtaGenMode ) {
    if ( ( m_minEta > m_requestedEta ) || ( m_maxEta < m_requestedEta ) || ( m_maxEta < m_minEta ) ) {
  ...
  if ( SPGGenMode::FixedMode != m_PhiGenMode ) {
    if ( ( m_minPhi > m_requestedPhi ) || ( m_maxPhi < m_requestedPhi ) || ( m_maxPhi < m_minPhi ) ) {
...
void GenericGun::generateParticle( Gaudi::LorentzVector& fourMomentum, Gaudi::LorentzVector& origin, int& pdgId,
  ...
  //
  double pt  = generateValue( m_PtGenMode, m_requestedPt, m_sigmaPt, m_minPt, m_maxPt, engine );
  double eta = generateValue( m_EtaGenMode, m_requestedEta, m_sigmaEta, m_minEta, m_maxEta, engine );
  double phi = generateValue( m_PhiGenMode, m_requestedPhi, m_sigmaPhi, m_minPhi, m_maxPhi, engine );
