...
// Constructor
//===========================================================================
Cosmics::Cosmics( const std::string& type, const std::string& name, const IInterface* parent )
...
StatusCode Cosmics::initialize() {
  ...
  info() << "Interaction region : ( " << m_minxvtx / Gaudi::Units::mm << " mm < x < " << m_maxxvtx / Gaudi::Units::mm
         << " mm"
  ...
  info() << "Momentum range: " << m_minMom / Gaudi::Units::GeV << " GeV <-> " << m_maxMom / Gaudi::Units::GeV << " GeV"
         << endmsg;
  info() << "Zenith angle range: " << m_minTheta / Gaudi::Units::rad << " rad <-> " << m_maxTheta / Gaudi::Units::rad
         << " rad" << endmsg;
...
//===========================================================================
void Cosmics::generateParticle( Gaudi::LorentzVector& fourMomentum, Gaudi::LorentzVector& origin, int& pdgId,
  ...
  for ( int i = 0; i < 1000000; i++ ) {
    ...
    if ( m_model == 1 ) {
      warning() << " You chose to use model 1; the use of this model is discouraged. You do it at your own risk "
                << endmsg;
      flux = sin( t ) * ( 18.0 * ( p + 5.0 ) * pow( p + 2.7 / cos( t ), -2.7 ) ) /
             ( ( p * cos( t ) + 145.0 ) * ( p + 5.0 / cos( t ) ) );
    } else if ( m_model == 0 ) {
  ...
  double phiprime = m_minPhi + flatGenerator() * ( m_maxPhi - m_minPhi );
  //
  ...
  */
  ...
  debug() << " p = " << momentum / Gaudi::Units::GeV << " GeV; "
          << " Phi = " << phi << endmsg;
  debug() << " Theta = " << theta * 180.0 / Gaudi::Units::pi << " degrees, " << theta << " rad"
          << " ( from t = " << t * 180.0 / Gaudi::Units::pi << " " << t << ")" << endmsg;
  ...
  fourMomentum.SetE( std::sqrt( m_masses[currentType] * m_masses[currentType] + fourMomentum.P2() ) );
  ...
  double       tofLight =
      sqrt( ( xprime - x ) * ( xprime - x ) + ( yprime - y ) * ( yprime - y ) + ( zprime - z ) * ( zprime - z ) ) /
      Gaudi::Units::c_light;
  ...
  debug() << " -> " << m_names[currentType] << endmsg << "   P   = " << fourMomentum << endmsg << "   Vtx = " << origin
          << endmsg;
}

double Cosmics::muonSpec( double muMomentum, double Theta, double vDepth, double overAngle ) {
