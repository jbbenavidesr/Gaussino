...
// Constructor
//============================================================================
MaterialEval::MaterialEval( const std::string& type, const std::string& name, const IInterface* parent )
...
StatusCode MaterialEval::initialize() {
  ...
  const LHCb::ParticleProperty* particle = ppSvc->find( LHCb::ParticleID( m_pdgCode ) );
  if ( !particle ) {
  ...
  if ( m_useGrid ) {
    ...
    if ( m_etaPhi ) {
      ...
      info() << "  with " << m_nEtastep << " steps in " << m_minEta << " <= eta <= " << m_maxEta << " and "
             << m_nPhistep << " steps in " << m_minPhi << " <= phi <= " << m_maxPhi << endmsg;
      info() << "  ==> DEta step = " << m_dEtastep << ", Dphi Step = " << m_dPhistep << endmsg;
    } else {
      m_dXstep = ( m_xmax - m_xmin ) / m_nXstep;
      m_dYstep = ( m_ymax - m_ymin ) / m_nYstep;
      info() << "x-y plane at z = " << m_zplane / Gaudi::Units::m << " m " << endmsg;
      info() << "  with " << m_nXstep << " steps in " << m_xmin / Gaudi::Units::cm
             << " cm <= x <= " << m_xmax / Gaudi::Units::cm << " cm and " << m_nYstep << " steps in "
             << m_ymin / Gaudi::Units::cm << " cm <= y <= " << m_ymax / Gaudi::Units::cm << " cm " << endmsg;
    ...
    if ( m_etaPhi ) {
      info() << "eta-phi plane" << endmsg;
      info() << "  with " << m_minEta << " <= eta <= " << m_maxEta << " and " << m_minPhi << " <= phi <= " << m_maxPhi
             << endmsg;
    } else {
      info() << "x-y plane at z = " << m_zplane / Gaudi::Units::m << " m " << endmsg;
      info() << "  with " << m_xmin / Gaudi::Units::cm << " cm <= x <= " << m_xmax / Gaudi::Units::cm << " cm and "
             << m_ymin / Gaudi::Units::cm << " cm <= y <= " << m_ymax / Gaudi::Units::cm << " cm " << endmsg;
    }
  }
...
void MaterialEval::generateParticle( Gaudi::LorentzVector& fourMomentum, Gaudi::LorentzVector& origin, int& pdgId,
  ...
  if ( m_useGrid ) {
    ...
    if ( m_etaPhi )
      generateUniformEtaPhi( px, py, pz, engine );
    else
      generateUniformXY( px, py, pz, engine );
  }
