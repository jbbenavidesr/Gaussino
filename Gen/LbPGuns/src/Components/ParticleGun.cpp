...
StatusCode ParticleGun::initialize() {
  ...
  if ( "" == m_particleGunToolName ) return Error( "No ParticleGun Generation Tool is defined. This is mandatory" );
  m_particleGunTool = tool<IParticleGunTool>( m_particleGunToolName, this );

  boost::char_separator<char>                   sep( "." );
  boost::tokenizer<boost::char_separator<char>> strList( m_particleGunTool->name(), sep );
  ...
  if ( "" != m_fullGenEventCutToolName )
    m_fullGenEventCutTool = tool<IFullGenEventCutTool>( m_fullGenEventCutToolName, this );
  ...
  always() << "==================================================================" << endmsg;
  always() << "Requested to generate EventType " << m_eventType << endmsg;
  always() << "==================================================================" << endmsg;
...
ParticleGun::operator()( const LHCb::GenHeader& theOldGenHeader ) const {
  ...
  if ( !theGenHeader.evType() ) { theGenHeader.setEvType( m_eventType ); }
  ...
  while ( !goodEvent ) {
    ...
    if ( 0 != m_numberOfParticlesTool ) nParticles = m_numberOfParticlesTool->numberOfPileUp( engine );
    ...
    for ( unsigned int i = 0; i < nParticles; ++i ) {
      ...
      theGenEvent->add_attribute( Gaussino::HepMC::Attributes::GaudiEventNumber,
                                  std::make_shared<HepMC3::IntAttribute>( theOldGenHeader.evtNumber() ) );
      theGenEvent->add_attribute( Gaussino::HepMC::Attributes::GaudiRunNumber,
                                  std::make_shared<HepMC3::IntAttribute>( theOldGenHeader.runNumber() ) );
      ...
      HepMC3::GenVertexPtr v{
          new HepMC3::GenVertex( HepMC3::FourVector( origin.X(), origin.Y(), origin.Z(), origin.T() ) ) };
      // create HepMC particle
      HepMC3::GenParticlePtr p{ new HepMC3::GenParticle(
          HepMC3::FourVector( theFourMomentum.Px(), theFourMomentum.Py(), theFourMomentum.Pz(), theFourMomentum.E() ),
          thePdgId, HepMC3::Status::StableInProdGen ) };
    ...
    if ( 0 != m_decayTool ) {
      ...
      for ( auto& event : theEvents ) {
        ...
        if ( m_genCutTool && theSignal ) {
          ++m_nBeforeCut;
          passCut = m_genCutTool->applyCut( theParticleList, theGenEvent.get(), theGenCollision );
...
StatusCode ParticleGun::finalize() {
  ...
  info() << "**************************************************" << endmsg;
  if ( 0 != m_numberOfParticlesTool ) m_numberOfParticlesTool->printPileUpCounters();
...
//=============================================================================
HepMC3::GenParticlePtr ParticleGun::decayEvent( HepMC3::GenEventPtr theEvent, ParticleVector& theParticleList,
                                                HepRandomEnginePtr& engine, StatusCode& sc ) const {
  ...
  for ( auto& thePart : particles ) {
    ...
    if ( ( HepMC3::Status::StableInProdGen == status ) ||
         ( ( HepMC3::Status::DecayedByDecayGenAndProducedByProdGen == status ) && ( 0 == thePart->end_vertex() ) ) ) {

      if ( m_decayTool->isKnownToDecayTool( thePart->pdg_id() ) ) {

        if ( HepMC3::Status::StableInProdGen == status )
          thePart->set_status( HepMC3::Status::DecayedByDecayGenAndProducedByProdGen );
        else
          thePart->set_status( HepMC3::Status::DecayedByDecayGen );
...
// Set up event
//=============================================================================
void ParticleGun::prepareInteraction( HepMC3::GenEventPtrs* theEvents, LHCb::GenCollisions* theCollisions,
