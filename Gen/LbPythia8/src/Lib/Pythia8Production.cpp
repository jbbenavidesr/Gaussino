...
StatusCode Pythia8Production::initialize() {
  ...
  std::string xmlpath( "UNKNOWN" != System::getEnv( "PYTHIA8XML" ) ? System::getEnv( "PYTHIA8XML" ) : "" );
  ...
  std::string        sm( "StandardModel:" ), mpi( "MultiPartonInteractions:" ), pre( "LhcbHooks:" ), parm( "pT0Ref" );
...
StatusCode Pythia8Production::initializeGenerator() {
  ...
  if ( m_lhaup ) {
    ...
    for ( unsigned int proc = 0; proc < procs.size(); ++proc ) {
      std::map<std::string, Pythia8::FVec> fvecs = m_pythia->settings.getFVecMap( procs[proc] );
      std::map<std::string, Pythia8::Flag> flags = m_pythia->settings.getFlagMap( procs[proc] );
      for ( std::map<std::string, Pythia8::FVec>::iterator itr = fvecs.begin(); itr != fvecs.end(); ++itr )
        m_pythia->settings.fvec( itr->first, std::vector<bool>( itr->second.valNow.size(), false ) );
      for ( std::map<std::string, Pythia8::Flag>::iterator itr = flags.begin(); itr != flags.end(); ++itr )
...
// Generate an event.
//=============================================================================
StatusCode Pythia8Production::generateEvent( HepMC3::GenEventPtr theEvent, LHCb::GenCollision* theCollision,
...
//=============================================================================
StatusCode Pythia8Production::toHepMC( HepMC3::GenEventPtr theEvent, LHCb::GenCollision* theCollision ) const {
...
// Update a particle.
//=============================================================================
void Pythia8Production::updateParticleProperties( const LHCb::ParticleProperty* thePP ) {
...
//=============================================================================
void Pythia8Production::turnOnFragmentation() { m_pythia->settings.flag( "HadronLevel:Hadronize", true ); }

void Pythia8Production::turnOffFragmentation() { m_pythia->settings.flag( "HadronLevel:Hadronize", false ); }

//=============================================================================
// Hadronize an event.
//=============================================================================
StatusCode Pythia8Production::hadronize( HepMC3::GenEventPtr theEvent, LHCb::GenCollision* theCollision,
...
//=============================================================================
void Pythia8Production::savePartonEvent( HepMC3::GenEventPtr /*theEvent*/ ) { m_event = m_pythia->event; }

//=============================================================================
// Retrieve the Pythia 8 event record.
//=============================================================================
void Pythia8Production::retrievePartonEvent( HepMC3::GenEventPtr /*theEvent*/ ) { m_pythia->event = m_event.get(); }
...
//=============================================================================
bool Pythia8Production::isSpecialParticle( const LHCb::ParticleProperty* thePP ) const {
