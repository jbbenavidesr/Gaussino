...
// local
#include "DaughtersInLHCbAndCutsForDstarDsnotfromB0.h"
...
//-----------------------------------------------------------------------------
// Implementation file for class : DaughtersInLHCbAndCutsForDstarDsnotfromB0
//
...
DECLARE_TOOL_FACTORY( DaughtersInLHCbAndCutsForDstarDsnotfromB0 )


//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
DaughtersInLHCbAndCutsForDstarDsnotfromB0::DaughtersInLHCbAndCutsForDstarDsnotfromB0( const std::string& type,
...
// Destructor 
//=============================================================================
DaughtersInLHCbAndCutsForDstarDsnotfromB0::~DaughtersInLHCbAndCutsForDstarDsnotfromB0( ) { ; }

//=============================================================================
// AndWithMinP function
//=============================================================================
bool DaughtersInLHCbAndCutsForDstarDsnotfromB0::applyCut( ParticleVector & theParticleVector ,
...
//=============================================================================
bool DaughtersInLHCbAndCutsForDstarDsnotfromB0::passCuts( const HepMC::GenParticle * theSignal ) const {
...
for ( Particles::const_iterator it = dstar_parents.begin() ; it != dstar_parents.end() ;
 ...
 if(abs((*it)->pdg_id())%10000>400&&abs((*it)->pdg_id())%10000<500)isel=1;
  
  if(abs((*it)->pdg_id())!=511&&abs((*it)->pdg_id())!=521&&abs((*it)->pdg_id())!=531&&abs((*it)->pdg_id())!=5122)continue;
  zB= (*it)->end_vertex()->position().z();
  iparent_dstar=abs((*it)->pdg_id());
break;
  }
 if(isel==1&&iparent_dstar!=511)isel=0;
 //we select D* not directly coming from B0 but ultimaletely coming from B0
 if( msgLevel (MSG::DEBUG))debug() << "N beautty in D* ancestors"<<bOK<<" vertex position "<<zB<< " D*parent "<<iparent_dstar<<" isel "<<isel<<endmsg;
...
for ( Particles::const_iterator it = unstables.begin() ; it != unstables.end() ;
 ...
 if ( 431 == abs((*it) -> pdg_id() ))nD++;
 //if ( 411 == abs((*it) -> pdg_id() ))nD++;
  
  }
 if(nD>0&&iparent_dstar!=511)isel=2;
 if( msgLevel (MSG::DEBUG))debug()<<"is there a Ds "<<nD<<" iparent_dstar " <<iparent_dstar<<" isel "<<isel<<endmsg;
