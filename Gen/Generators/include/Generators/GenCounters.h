...
namespace GenCounters {
  ...
  enum excitedCounterType {
    _0star, ///< counter of X (spin 0, ang mom 0)
   ...
   *  @return A/B
   */
  inline double fraction( const unsigned int A, const unsigned int B ) { return ( (double)A / (double)B ); }
   ...
   */
  inline void printEfficiency( MsgStream& theStream, const std::string& cutName, const unsigned int after,
                               const unsigned int before ) {
    if ( 0 == before ) return;
    theStream << "Number of events for " << cutName << ", before : " << before << ", after : " << after << std::endl;
    theStream << "Efficiency of the " << cutName << " : "
              << format( "%.5g +/- %.5g", fraction( after, before ), err_fraction( after, before ) ) << std::endl;
  }
   ...
   *  @param[in] before     Number of events before the cut
   */
  inline void printEfficiency( ICounterLogFile* theLogFile, const std::string& cutName, const unsigned int after,
                               const unsigned int before ) {
    if ( 0 == before ) return;
    theLogFile->addEfficiency( cutName, after, before, fraction( after, before ), err_fraction( after, before ) );
  }
   ...
   */
  inline void printCounter( MsgStream& theStream, const std::string& name, const unsigned int value ) {
   ...
   *  @param[in] before     Number of events before the cut
   */
  inline void printCounter( ICounterLogFile* theLogFile, const std::string& name, const unsigned int value ) {
   ...
   */
  inline void printFraction( MsgStream& theStream, const std::string& name, const unsigned int number,
                             const unsigned int total ) {
    theStream << "Number of " << name << " : " << number;
    theStream << " [fraction : " << format( "%.5g +/- %.5g]", fraction( number, total ), err_fraction( number, total ) )
              << std::endl;
  }
   ...
   *  @param[in] total      Total number of particles.
   */
  inline void printFraction( ICounterLogFile* theLogFile, const std::string& name, const unsigned int number,
                             const unsigned int total ) {
    theLogFile->addFraction( name, number, fraction( number, total ), err_fraction( number, total ) );
  }
  ...
  inline void AddTo( std::array<T, N>& A, const std::array<T, N>& B ) {
    std::transform( A.begin(), A.end(), B.begin(), A.begin(), std::plus<unsigned int>() );
  }
   ...
   */
  template <typename T, std::size_t N>
  inline void printArray( MsgStream& theStream, const std::array<T, N>& A, const std::array<std::string, N>& AName,
                          const std::string& root ) {
    unsigned int total = std::accumulate( A.begin(), A.end(), 0 );
    for ( unsigned int i = 0; i < A.size(); ++i ) printFraction( theStream, root + " " + AName[i], A[i], total );
  }
   ...
   */
  template <typename T, std::size_t N>
  inline void printArray( ICounterLogFile* theLogFile, const std::array<T, N>& A,
                          const std::array<std::string, N>& AName, const std::string& root ) {
    unsigned int total = std::accumulate( A.begin(), A.end(), 0 );
    for ( unsigned int i = 0; i < A.size(); ++i ) printFraction( theLogFile, root + " " + AName[i], A[i], total );
  }
   ...
   */
  void setupBHadronCountersNames( BHadronCNames& BC, BHadronCNames& antiBC );
   ...
   */
  void setupDHadronCountersNames( DHadronCNames& DC, DHadronCNames& antiDC );
   ...
   */
  void setupExcitedCountersNames( ExcitedCNames& B, const std::string& root );
   ...
   */
  void updateExcitedStatesCounters( const HepMC3::GenEvent* theEvent, ExcitedCounter& thebExcitedC,
   ...
   */
  void updateHadronCounters( const HepMC3::GenEvent* theEvent, BHadronCounter& thebHadC, BHadronCounter& theantibHadC,
                             DHadronCounter& thecHadC, DHadronCounter& theanticHadC, std::atomic_uint& thebbCounter,
                             std::atomic_uint& theccCounter );

  void updateHadronFSR( const HepMC3::GenEvent* theEvent, LHCb::GenFSR* genFSR, const std::string option );
