...
class ExternalGenerator : public GaudiTool, public ISampleGenerationTool {
public:
  ...
  ExternalGenerator( const std::string& type, const std::string& name, const IInterface* parent );
...
protected:
   ...
   *                                decay table).
   */
  StatusCode decayHeavyParticles( HepMC3::GenEventPtr theEvent, const LHCb::ParticleID::Quark theQuark,
   ...
   */
  bool checkPresence( const PIDs& pidList, HepMC3::GenEventPtr theEvent, ParticleVector& particleList ) const;
...
private:
   ...
   */
  StatusCode parseLhaPdfCommands( const CommandVector& theCommandVector ) const;
