...
// ============================================================================
StatusCode F77Utils::open( int lun, const std::string& file, const bool verbose ) {
  return openUnknown( lun, file, verbose );
}
// ============================================================================
// Open "FORMATTED" Fortran file with status="UNKNOWN"
// ============================================================================
StatusCode F77Utils::openFormatted( int lun, const std::string& file, const bool verbose ) {
...
StatusCode F77Utils::close( int lun, const bool verbose ) {
  ...
  if ( result != lun ) {
    std::cerr << "F77Utils::close()               ERROR Error in closing LUN=" << lun << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::close()               INFO  LUN=" << lun << " is closed " << std::endl;
  }
...
// ==========================================================================
StatusCode F77Utils::openNew( int lun, const std::string& file, const bool verbose ) {
  const int result = f77onfile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openNew               ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openNew               INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
...
// ==========================================================================
StatusCode F77Utils::openOld( int lun, const std::string& file, const bool verbose ) {
  const int result = f77oofile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openOld               ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openOld               INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
...
// Open Fortran file with status="UNKNOWN"
// ==========================================================================
StatusCode F77Utils::openUnknown( int lun, const std::string& file, const bool verbose ) {
  const int result = f77oufile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openUnknown           ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openUnknown           INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
...
// ==========================================================================
StatusCode F77Utils::openNewFormatted( int lun, const std::string& file, const bool verbose ) {
  const int result = f77onffile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openNewFormatted      ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openNewFormatted      INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
...
// ==========================================================================
StatusCode F77Utils::openOldFormatted( int lun, const std::string& file, const bool verbose ) {
  const int result = f77ooffile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openOldFormatted      ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openOldFormatted      INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
...
// Open "FORMATTED" Fortran file with status="UNKNOWN"
// ==========================================================================
StatusCode F77Utils::openUnknownFormatted( int lun, const std::string& file, const bool verbose ) {
  const int result = f77ouffile( &lun, file.c_str(), file.size() );
  if ( result != lun ) {
    std::cerr << "F77Utils::openUnknownFormatted  ERROR Error in opening LUN=" << lun << " FILE='" << file << "' "
              << std::endl;
  } else if ( verbose ) {
    std::cout << "F77Utils::openUnknownFormatted  INFO  LUN=" << lun << " FILE='" << file << "' is opened "
              << std::endl;
  }
  return result != lun ? StatusCode::FAILURE : StatusCode::SUCCESS;
}
