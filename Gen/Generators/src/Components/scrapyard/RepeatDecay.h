 ...
 *  @date   2007-03-31
 */
class RepeatDecay : public GaudiTool, virtual public ISampleGenerationTool {
private:
...
public:
  ...
  RepeatDecay( const std::string& type, const std::string& name, const IInterface* parent );
...
private:
  ...
  void copyEvents( HepMC3::GenEventPtrs& from, HepMC3::GenEventPtrs& to );

  /* Copy a set of collisions into another, with the list of corresponding
   * events
   */
  void copyCollisions( LHCb::GenCollisions& from, LHCb::GenCollisions& to, HepMC3::GenEventPtrs& theEvents );
};
