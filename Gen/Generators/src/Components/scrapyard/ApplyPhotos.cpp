...
//=============================================================================
ApplyPhotos::ApplyPhotos( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
...
StatusCode ApplyPhotos::initialize() {
  ...
  if ( m_pdgIdList.empty() ) return Error( "PDGId list property is not set" );

  for ( std::vector<int>::iterator it = m_pdgIdList.begin(); it != m_pdgIdList.end(); ++it ) m_pdgIds.insert( *it );
...
StatusCode ApplyPhotos::execute() {
  ...
  LHCb::HepMCEvents* theEvents = get<LHCb::HepMCEvents>( m_hepMCEventLocation );

  LHCb::HepMCEvents::iterator it;

  for ( it = theEvents->begin(); it != theEvents->end(); ++it ) {
    HepMC3::GenEventPtr ev = ( *it )->pGenEvt();
    for ( HepMC3::GenEvent::particle_iterator itP = ev->particles_begin(); itP != ev->particles_end(); ++itP ) {
      if ( LHCb::HepMCEvent::DocumentationParticle != ( *itP )->status() ) {
        if ( std::binary_search( m_pdgIds.begin(), m_pdgIds.end(), abs( ( *itP )->pdg_id() ) ) ) {
          ...
          for ( iterDes = EV->particles_begin( HepMC3::descendants );
            ...
            if ( 0 != ( *iterDes )->end_vertex() ) newEvent->add_vertex( ( *iterDes )->end_vertex() );
          }
          ...
          for ( iterDes = EV->particles_begin( HepMC3::descendants );
                iterDes != EV->particles_end( HepMC3::descendants ); ++iterDes ) {
            if ( 0 != ( *iterDes )->end_vertex() ) ev->add_vertex( ( *iterDes )->end_vertex() );
          }
        }
      }
    }
