...
//=============================================================================
bool SignalForcedFragmentation::generate( const unsigned int nPileUp, HepMC3::GenEventPtrs& theEvents,
  ...
  theSignalAtRest->set_momentum( HepMC3::FourVector( 0., 0., 0., m_signalMass ) );
  ...
  HepMC3::GenVertex* theVertex = new HepMC3::GenVertex( HepMC3::FourVector( 0., 0., 0., 0. ) );
  ...
  sc = m_productionTool->setupForcedFragmentation( theSignalAtRest->pdg_id() );
    ...
    if ( !result ) {
      ...
      if ( checkPresence( signalPid, theGenEvent, theParticleList ) ) {
        ...
        HepMC3::GenParticlePtr theSignal =
            chooseAndRevert( theParticleList, isInverted, dummyHasFlipped, hasFailed, engine );
        ...
        if ( passCut && ( !theParticleList.empty() ) ) {
          ...
          updateCounters( theParticleList, m_nParticlesAfterCut, m_nAntiParticlesAfterCut, true, false );
          ...
          // Update counters
          GenCounters::updateHadronCounters( theGenEvent.get(), m_bHadC, m_antibHadC, m_cHadC, m_anticHadC, m_bbCounter,
                                             m_ccCounter );
...
//=============================================================================
StatusCode SignalForcedFragmentation::boostTree( HepMC3::GenParticlePtr      theSignal,
                                                 HepMC3::ConstGenParticlePtr theSignalAtRest,
                                                 const ROOT::Math::Boost&    theBoost ) const {
  if ( !theSignalAtRest->end_vertex() ) return StatusCode::SUCCESS;

  if ( theSignal->end_vertex() ) return Error( "The particle has already a decay vertex !" );

  if ( !theSignalAtRest->production_vertex() ) return Error( "The particle has no production vertex !" );

  // Displacement in original frame
  Gaudi::LorentzVector positionEnd, positionBegin;
  positionEnd.SetXYZT( theSignalAtRest->end_vertex()->position().x(), theSignalAtRest->end_vertex()->position().y(),
                       theSignalAtRest->end_vertex()->position().z(),
                       theSignalAtRest->end_vertex()->position().t() * Gaudi::Units::c_light );
  ...
  positionBegin.SetXYZT( theSignalAtRest->production_vertex()->position().x(),
                         ...
                         theSignalAtRest->production_vertex()->position().t() * Gaudi::Units::c_light );
  ...
  newP.SetXYZT( newPosition.X(), newPosition.Y(), newPosition.Z(), newPosition.T() / Gaudi::Units::c_light );

  // Add original position
  Gaudi::LorentzVector originalPosition( theSignal->production_vertex()->position() );
  newP += originalPosition;

  // Create new HepMC vertex after boost and add it to the current event
  HepMC3::GenVertex* newVertex = new HepMC3::GenVertex( HepMC3::FourVector( newP.X(), newP.Y(), newP.Z(), newP.T() ) );
  ...
  for ( auto child : HepMC3::Relatives::CHILDREN( theSignalAtRest ) ) {
    ...
    HepMC3::GenParticlePtr newPart{ new HepMC3::GenParticle(
        HepMC3::FourVector( newMomentum.Px(), newMomentum.Py(), newMomentum.Pz(), newMomentum.E() ), id, status ) };
