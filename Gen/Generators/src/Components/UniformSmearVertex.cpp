...
//=============================================================================
UniformSmearVertex::UniformSmearVertex( const std::string& type, const std::string& name, const IInterface* parent )
    : GaudiTool( type, name, parent ), m_rmaxsq( 0. ), m_deltaz( 0. ) {
...
StatusCode UniformSmearVertex::initialize() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << " with r less than " << m_rmax / Gaudi::Units::mm << " mm." << endmsg;
    debug() << " with z between " << m_zmin / Gaudi::Units::mm << " mm and " << m_zmax / Gaudi::Units::mm << " mm."
            << endmsg;
  } else {
    info() << " with r <= " << m_rmax / Gaudi::Units::mm << " mm, " << m_zmin / Gaudi::Units::mm
           << " mm <= z <= " << m_zmax / Gaudi::Units::mm << " mm." << endmsg;
  }
