...
extern "C" {
  extern struct {
    ...
    float typevt; // ........ type of event (1=Non Diff, 2=Double Diff, 
    // 3=Central Diff, 4=AB->XB, -4=AB->AX)
  } c2evt_;       // epos.inc
}
...
void CRMCWrapper::set_high_energy_model(int new_model_id) {
  ...
  if (new_model_id == crmc_generators::PYTHIA || 
      new_model_id == crmc_generators::HIJING) {
    std::cerr << "---------------------------------------------------" << std::endl;
    std::cerr << "LbCRMC : ERROR : CRMC's PYTHIA and Hijing are " 
              << "disabled not to interfere with LHCb's versions!" << std::endl;
    std::cerr << "---------------------------------------------------" << std::endl;
    std::string message = std::string( "CRMC's versions of PYTHIA and Hijing " ) 
      + std::string( "generators are disabled by default in LHCb SW!" ) ;
    throw std::runtime_error( message.c_str() ) ;
  }

  // Check if the provided generator ID makes sense
  if (!((new_model_id >= crmc_generators::EPOS_LHC && 
         new_model_id <= crmc_generators::PHOJET) || 
        new_model_id == crmc_generators::DPMJET)) {
    std::cerr << "--------------------------------------------------" << std::endl;
...
// Produce event
//=================================================================================
void CRMCWrapper::generate_event(HepMC::GenEvent *event , bool fill) {
...
void CRMCWrapper::fill_event(HepMC::GenEvent *event ) {
  if (!event)
    throw
      std::runtime_error("LbCRMC : Null pointer for the event passed to fill_event()!");
...
void CRMCWrapper::correct_particle_energy(HepMC::GenEvent *event) {
  ...
  for (HepMC::GenEvent::particle_iterator particle = event->particles_begin(); 
...
void CRMCWrapper::convert_to_mev_and_mm(HepMC::GenEvent *event) {
  if (!event)
    throw 
      std::runtime_error("LbCRMC : Null pointer for the event passed to convert_to_mev_and_mm()!");

  // Scale factor for momentum
  double momentum_scale_factor = 1.;
  momentum_scale_factor = 
    HepMC::Units::conversion_factor(HepMC::Units::GEV, event->momentum_unit());
  ...
  length_scale_factor  = 
    HepMC::Units::conversion_factor(HepMC::Units::MM, event->length_unit());

  // Adjust length
  if (length_scale_factor != 1.) {
    for (HepMC::GenEvent::vertex_iterator vertex = event->vertices_begin(); 
         vertex != event->vertices_end(); ++vertex) {
...
void CRMCWrapper::reinitialize_random_number_generator(double new_seed) {
  if (m_generator_ready && (fHEModel == crmc_generators::EPOS_LHC || 
                            fHEModel == crmc_generators::EPOS_199)) {
...
void CRMCWrapper::print_event(HepMC::GenEvent *event) {
  if (!event)
    throw 
      std::runtime_error("LbCRMC : Null pointer for the event passed to print_event()!");
