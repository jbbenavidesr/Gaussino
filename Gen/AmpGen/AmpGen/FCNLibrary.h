...
class FCNLibrary  {
  ...
  public: 
    ...
    void add( CompiledExpression<RETURN_TYPE>& expression ){
      ...
      if( it == m_objects.end() ) {
        m_objects.insert( 
            std::pair<unsigned int, CompiledExpression<RETURN_TYPE> &>( expression.hash(),expression ));
    ...
    void add ( std::vector<CompiledExpression<RETURN_TYPE>>& expressions ){
      for( auto& expression : expressions ) add( expression );
    }
    ...
    virtual bool compile(const std::string& name="/tmp/functions", bool debug=false){
      ...
      const std::string buildCommand = 
        "g++ -Ofast -shared -rdynamic -fPIC -ftree-vectorize -ftree-vectorizer-verbose=2 "+name+".cpp -o " + name + ".so";
