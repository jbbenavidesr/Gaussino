...
MinuitParameter* Minimiser::getParPtr(unsigned int i){
  if(0 == _parSet) return 0;
  if(i >= nPars()) return 0;
  return _parSet->getParPtr(i);
}
const MinuitParameter* Minimiser::getParPtr(unsigned int i) const{
  if(0 == _parSet) return 0;
  if(i >= nPars()) return 0;
  return _parSet->getParPtr(i);
}
...
TGraph* Minimiser::scan(MinuitParameter& fp, double from, double to){
...
TMatrixTSym<double> Minimiser::covMatrix() {
  ...
  for(unsigned int i=0; i < internalPars; i++){
    for(unsigned int j=0; j < internalPars; j++){
      m1[i + internalPars*j]=0.0;
    }
  }
  this->mnemat(&m1[0], internalPars);
  TMatrixTSym<double> matrix(internalPars);
  for(unsigned int i=0; i < internalPars; i++){
    for(unsigned int j=i; j < internalPars; j++){
      matrix(i,j) = matrix(j,i) = m1[i+j*internalPars];
    }
  }
  return matrix;
}
...
TMatrixTSym<double> Minimiser::covMatrixFull() {
  ...
  for(unsigned int i=0; i < internalPars; i++){
    for(unsigned int j=i; j < internalPars; j++){
      ...
      matrix(ex_i,ex_j) = matrix(ex_j,ex_i) = m1[i+j*internalPars];
    }
  }
  return matrix;
}
