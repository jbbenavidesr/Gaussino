...
template <> Factory<IExtendLikelihood>* Factory<IExtendLikelihood>::gImpl = 0;

double GaussianConstraint::getVal() const {
 return - ( m_param->mean() - m_mean ) * ( m_param->mean() - m_mean )/ ( 2 * m_sigma * m_sigma ); 
}

void GaussianConstraint::configure( const std::string& configString, const AmpGen::MinuitParameterSet& mps ){
  ...
  if( tokens.size() == 4 ){
    ...
    if( tokens[2] == "PDG" ){
      ...
      auto particle_properties = ParticlePropertiesList::getMe()->get( split_tokens[0] );
