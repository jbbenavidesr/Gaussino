...
DECLARE_ALGORITHM_FACTORY( OTCosmicGenerator );

//--------------------------------------------------------------------------
OTCosmicGenerator::OTCosmicGenerator(const std::string& name, 
...
//--------------------------------------------------------------------------
 OTCosmicGenerator::~OTCosmicGenerator()
//--------------------------------------------------------------------------
{
}

StatusCode OTCosmicGenerator::initialize() 
{
...
StatusCode OTCosmicGenerator::callParticleGun( HepMC::GenEvent * evt ) 
{
  ++m_events;
  
  debug() << "Event #" << m_events << endmsg;
  bool accepted=false;
  HepLorentzVector vertex, p4 ;
  ...
  while(!accepted){
    ...
    double phi1   = M_PI * (2*m_flatgenerator()-1) ;
    ...
    const double b       = 4e-6 ; // g^{-1} * cm^2
    const double eps     = a/b ;  // ~ 600 GeV
    const double depth   = 1e4 ;  // cm
    ...
    if(Emu > 200*Gaudi::Units::MeV) {
      ...
      p4 =  HepLorentzVector(Emu*sin(theta1)*cos(phi1),-Emu*cos(theta1),Emu*sin(theta1)*sin(phi1),Emu);
      //p4 *= Gaudi::Units::GeV ;
      Gaudi::XYZVector globaldir(p4.x(),p4.y(),p4.z()) ;
	...
	vertex = HepLorentzVector(globalpostop.x(), globalpostop.y(), globalpostop.z(), CLHEP::Tcomponent(time) ) ;
	accepted = true ;
  ...
  // Add the vertex
  HepMC::GenVertex * v1 = new HepMC::GenVertex( vertex );
  evt->add_vertex( v1 );
  int charge = int(m_flatgenerator() * 2 - 1);//m_cosmicgun->GetMuonCharge();
  v1 ->add_particle_out( new HepMC::GenParticle( p4 , charge > 0 ? m_muplus->pdgID()  : m_muminus->pdgID(),
...
//---------------------------------------------------------------------------
StatusCode OTCosmicGenerator::finalize() 
{
