...
namespace Gsino::ML::Torch {
  ...
  struct ModelServerSvc : public extends<Service, IModelServerSvc<ModelServer>> {
    ...
    StatusCode initialize() override {
      return extends::initialize().andThen( [&]() -> StatusCode {
        ...
        m_server->printThreadInfo();

        m_server->load( m_modelPath );
  ...
  private:
    ...
    Gaudi::Property<std::string> m_modelName{ this, "ModelName", "UnspecifiedTorchModel" };
    ...
    Gaudi::Property<size_t>      m_intraopThreads{ this, "IntraOpThreads", 0,
                                              "Number of intra-op threads (0 -> pyTorch default)" };
    Gaudi::Property<size_t>      m_interopThreads{ this, "InterOpThreads", 0,
                                              "Number of inter-op threads (0 -> pyTorch default)" };
  };
} // namespace Gsino::ML::Torch

DECLARE_COMPONENT( Gsino::ML::Torch::ModelServerSvc )
