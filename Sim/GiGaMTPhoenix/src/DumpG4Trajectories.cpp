...
namespace GiGa::Phoenix {
  class DumpG4Trajectories : public Gaudi::Functional::Consumer<void( const G4EventProxies& )> {
    ...
    Gaudi::Property<std::vector<std::string>> m_trajectory_properties{this, "TrajectoryProperties", {"charge", "mom", "pT", "eta"}};
    Gaudi::Property<std::string> m_trajectory_model{this, "TrajectoryModel", "DrawByCharge"};
    Gaudi::Property<std::map<std::string, std::string>> m_charge_colors{this, "ChargeColors", {{ "Positive", "0000FF" }, { "Negative", "FF0000" }, { "Neutral", "00FF00"}}};
    Gaudi::Property<std::map<std::string, std::string>> m_particle_id_colors{this, "ParticleIDColors", {{"gamma", "00FF00"}, {"e-", "FF0000"}, {"e+", "0000FF"}, {"pi-", "FF00FF"}, {"pi+", "FF00FF"}, {"proton", "00FFFF"}}};
    Gaudi::Property<std::map<std::string, std::pair<double, double>>> m_interval_colors{this, "IntervalColors", {{"FFAA00", {0 * Gaudi::Units::MeV, 25 * Gaudi::Units::MeV}}, {"00AA00", {25 * Gaudi::Units::MeV, 100 * Gaudi::Units::MeV}}, {"000033", {150 * Gaudi::Units::MeV, 1000 * Gaudi::Units::MeV}}}};
  ...
  public:
    ...
    void operator()( const G4EventProxies& eventproxies ) const override {
      for ( auto& ep : eventproxies ) {
        ...
        for ( auto& trajectory : *( trajCont->GetVector() ) ) {
          ...
          if ( !std::any_of(m_accepted_charges.value().begin(), m_accepted_charges.value().end(), [&charge](double i){return i == charge;})) continue;

          nlohmann::json available_trajectory_properties {{"charge", (int) std::round(charge)}, {"mom", init_p}, {"pT", init_pt}, {"id", pid}, {"eta", init_eta}};
          ...
          for( auto& key: m_trajectory_properties ){
            ...
            if (property != available_trajectory_properties.end()) {
              used_trajectory_properties[key] = *property;
            }
          }
    ...
    std::string GetTrajectoryColor( G4VTrajectory* traj ) const {
      if (m_trajectory_model.value() == "drawByCharge"){
        return GetTrajectoryColorByCharge( traj );
      }
      if (m_trajectory_model.value() == "drawByParticleID") {
        return GetTrajectoryColorByParticleID( traj );
      }
