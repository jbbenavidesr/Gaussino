...
ReDecaySimAlg::operator()( const HepMC3::GenEventPtrs& originalhepmcevents,
                           const HepMC3::GenEventPtrs& signalhepmcevents, const Gsino::SimHeader& simheader ) const {
  ...
  if ( m_redecaysvc->isCurrentOriginal() ) {
    ...
    ret_tuple = m_gigaSvc->simulate( simheader.evtNumber(), originalhepmcevents, engine );
  ...
  for ( auto& evt : signalhepmcevents ) {
    for ( auto& p : evt->particles() ) {
      if ( auto orgattr =
        ...
        auto [g4proxy, sigmctruth] = m_gigaSvc->simulateDecay( simheader.evtNumber(), p, engine );
