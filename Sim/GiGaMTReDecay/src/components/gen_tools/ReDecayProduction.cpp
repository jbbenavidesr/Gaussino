 ...
 * @date   2020-02-19
 */
class ReDecayProduction : public extends<GaudiTool, IProductionTool> {
public:
    using extends::extends;
    /// Set of functions we need for this
    StatusCode generateEvent(HepMC3::GenEventPtr,
                             LHCb::GenCollision*,
    ...
    StatusCode hadronize(HepMC3::GenEventPtr,
                         LHCb::GenCollision*,
...
StatusCode ReDecayProduction::generateEvent(
    HepMC3::GenEventPtr theEvent, LHCb::GenCollision*,
  ...
  for (auto partid : redids) {
    ...
    auto v = std::make_shared<HepMC3::GenVertex>(
        HepMC3::FourVector(origin.x(), origin.y(), origin.z(), origin.t()));
    if (dummy_vertex == nullptr) {
      dummy_vertex = std::make_shared<HepMC3::GenVertex>(
          HepMC3::FourVector(origin.x(), origin.y(), origin.z(), origin.t()));
      auto beam =
          std::make_shared<HepMC3::GenParticle>(HepMC3::FourVector(0, 0, 1, 1), 0,
                                 LHCb::HepMCEvent::DocumentationParticle);
      dummy_vertex->add_particle_in(beam);
      theEvent->add_vertex(dummy_vertex);
      if (theEvent->beams().size()==0) {
    ...
    auto tag = std::make_shared<HepMC3::GenParticle>(
        HepMC3::FourVector(mom.px(), mom.py(), mom.pz(), mom.e()), 0,
        LHCb::HepMCEvent::StableInProdGen);
