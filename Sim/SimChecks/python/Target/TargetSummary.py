...
class Plotter:
    ...
    def __init__(self, path="", outputPath="", models=[], pguns=[], energies=[], materials=[], thicks=[], doprint=False):
        ...
        os.system("mkdir -p {}/ROOTFiles".format(outputPath))
        rootFileName = os.path.join(outputPath, "ROOTFiles/TargetsPlots.root")
        rootin = TFile(os.path.join(path, "TargetOutput.root"))
        ...
        for m in range(0, len(self._models)):
            ...
            xsec_txtout = open(os.path.join(outputPath, "data_tables/XsecSummary_" + self._models[m] + ".txt"), "w")
            multi_txtout = open(os.path.join(outputPath, "data_tables/MultiSummary_" + self._models[m] + ".txt"), "w")
