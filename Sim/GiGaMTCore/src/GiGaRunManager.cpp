...
GiGaRunManager::GiGaRunManager()
  : G4RunManager()
  , m_msg("GiGaRunManager")
  , m_recordFlux(false)
  , m_senDetTool("SensitiveDetectorMasterTool")
  , m_fastSimTool("FastSimulationMasterTool")
  , m_physListTool("PhysicsListToolBase")
  , m_userActionSvc("G4UA::UserActionSvc", "GiGaRunManager")
  , m_detGeoSvc("DetectorGeometrySvc", "GiGaRunManager")
{  }


GiGaRunManager* GiGaRunManager::GetGiGaRunManager()
{
  static GiGaRunManager *thisManager = nullptr;
  if (!thisManager) { thisManager = new GiGaRunManager; } // Leaked
  return thisManager;
}
...
{
  const std::string methodName = "GiGaRunManager::Initialize";
...
void GiGaRunManager::InitializeGeometry()
{
  ...
  if (m_detGeoSvc.retrieve().isFailure()) {
    ...
    G4Exception("GiGaRunManager", "CouldNotRetrieveDetGeoSvc", FatalException, description);
  ...
  if (m_senDetTool.retrieve().isFailure()) { //svcLocator->service("SensitiveDetector",m_senDetSvc).isFailure())
    ...
    G4Exception("GiGaRunManager", "CouldNotRetrieveSDMaster", FatalException, description);
    abort(); // to keep Coverity happy
  }
  if(m_senDetTool->initializeSDs().isFailure()) {
    ...
    G4Exception("GiGaRunManager", "FailedToInitializeSDs", FatalException, description);
    abort(); // to keep Coverity happy
  }
  return;
}
...
void GiGaRunManager::EndEvent()
{
  ATH_MSG_DEBUG( "GiGaRunManager::EndEvent" );
}


void GiGaRunManager::InitializePhysics()
{
  ...
  if (m_physListTool.retrieve().isFailure()) {
    ...
    G4Exception("GiGaRunManager", "CouldNotRetrievePLTool", FatalException, description);
    abort(); // to keep Coverity happy
  }
  ...
  if (m_fastSimTool.retrieve().isFailure()) {
    ...
    G4Exception("GiGaRunManager", "CouldNotRetrieveFastSimMaster", FatalException, description);
    abort(); // to keep Coverity happy
  }
  if(m_fastSimTool->initializeFastSims().isFailure()) {
    ...
    G4Exception("GiGaRunManager", "FailedToInitializeFastSims", FatalException, description);
    abort(); // to keep Coverity happy
  }
...
void GiGaRunManager::InitializeFluxRecording()
{
...
bool GiGaRunManager::ProcessEvent(G4Event* event)
{
  ...
  if (currentEvent->IsAborted()) {
    ATH_MSG_WARNING( "GiGaRunManager::ProcessEvent: Event Aborted at Detector Simulation level" );
  ...
  if (currentEvent->IsAborted()) {
    ATH_MSG_WARNING( "GiGaRunManager::ProcessEvent: Event Aborted at Analysis level" );
    currentEvent = nullptr;
    return true;
  }
...
void GiGaRunManager::RecordFlux()
{
  G4ScoringManager* ScM = G4ScoringManager::GetScoringManagerIfExist();
  if(ScM) {
    G4int nPar = ScM->GetNumberOfMesh();
...
void GiGaRunManager::RunTermination()
{
  ATH_MSG_DEBUG( " GiGaRunManager::RunTermination() " );
  if (m_recordFlux) {
...
void GiGaRunManager::WriteFluxInformation()
{
  G4UImanager *ui=G4UImanager::GetUIpointer();
