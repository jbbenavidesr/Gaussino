...
namespace Gsino::CaloChallenge {
  /**
   * @brief Represents a calo hit (Geant4)
   *
   ...
   * @date 2023
   */
  struct CaloG4Hit : public Gaussino::HitBase {
    ...
    /// Set relative position (to be used for drawing with Geant4)
    inline void SetPos( G4ThreeVector aXYZ ) { fPos = aXYZ; }
    /// Get relative position (to be used for drawing with Geant4)
    inline G4ThreeVector GetPos() const { return fPos; }
    /// Set absolute position (to be used for hit anchoring in the event)
    inline void SetPosAbs( G4ThreeVector aXYZ ) { fPosAbs = aXYZ; }
    /// Get absolute position (to be used for hit anchoring in the event)
    inline G4ThreeVector GetPosAbs() const { return fPosAbs; }
    /// Set rotation
